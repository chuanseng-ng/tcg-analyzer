...
Four closed lists, and nothing else. An analysis has a state, a session has a
state, an image has a side and a quality verdict; what *moves* between those
states is issue #35's state machine, and what an analysis *is* belongs to the
issues that give it behaviour. This module only fixes the words, so that the
database's CHECK constraints, the API and the job runner cannot each spell them
differently.
