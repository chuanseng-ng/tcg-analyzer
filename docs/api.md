...
Every response carries an `X-Request-Id`, generated by the service and never
accepted from the client, and every log line the request wrote carries the same
id. Quote it when reporting a failure: it is the join key between what the
browser saw and what the service logged, and it names nothing about the request
itself.
