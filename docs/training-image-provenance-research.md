...
| R5 | 5 | ~~No consent mechanism exists, V1 has no accounts (§53), and §54's sweep deletes every uploaded image when its session expires~~ **Closed 2026-09-10 by #148**, which asks per analysis with no account and retains by copying into the corpus rather than by exempting a sweep | no longer — the class supplies images at whatever volume the product sees | Built. See ADR 0008's addendum and `docs/retention.md`'s second exemption |
