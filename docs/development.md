...
`.env` is what a host-run process reads (`uv run uvicorn`, the CLIs); the
Compose stack reads the `.env` beside `infrastructure/local/docker-compose.yml`
and nothing else — see [Logs](#logs) for the one setting most often overridden
there.
...
- **A dead-lettered job records the job id, the analysis id, the error and the
  attempt count — and nothing else.** Analysis payloads reference photographs of
  somebody's card and their surroundings; keeping one indefinitely so a job
  nobody re-drives could be re-driven is not a trade this project makes (spec
  §54).
...
JSON by default (`TCG_API_LOG_FORMAT=json`, the `Settings` default, and what
the Compose stack ships). A developer who wants the coloured console rendering
opts in with `TCG_API_LOG_FORMAT=console` — in the root `.env` for a host-run
process, and in the `.env` beside `infrastructure/local/docker-compose.yml` for
the stack, which is the only file Compose reads. Nothing forces `console` any
more; JSON is what an operator gets from `docker compose logs`.

Every line carries `event`, `level`, `logger` and an ISO `timestamp`. Two ids
join lines into stories, bound once and merged into every line by structlog's
contextvars:

- **`request_id`** — on every line a request writes, generated by the API and
  echoed as `X-Request-Id`, never accepted from the client. The request's own
  line is `api.request_completed`: `method`, `route` (the template,
  `/analyses/{analysis_id}` — never the path, which names a user's analysis),
  `status` and `duration_ms`. A 404 has `route: null`.
- **`analysis_id` and `job_id`** — on every line a worker run writes. The run's
  own lines are `analysis.step_completed` (`step` ∈ `versions`, `gate`,
  `condition`, `grading`, with `duration_ms`), `image.assessed` per side
  (`side`, `cached`, `duration_ms` — the cache-hit rate is a count over
  `cached`), and `analysis.job_finished` (`outcome` ∈ `awaiting_confirmation`,
  `failed`, and `total_ms` for the whole task). A run that dies is
  `analysis.dead_lettered`, joinable by `analysis_id` since #266.

What spec §67 asks the product to track, and where each is read from:

| Signal | Event | Field |
| --- | --- | --- |
| analysis latency | `analysis.step_completed` | the sum of `duration_ms` over a run's steps (claim → `awaiting_confirmation`) |
| image-processing latency | `image.assessed` | `duration_ms`, beside `cached` |
...
```bash
grep '"analysis_id": "<id>"' | jq -c '{timestamp, event, request_id, step, side, cached, duration_ms, outcome, reason}'
```

The first line is `analysis.queued`, carrying the `request_id` of the `POST
/analyses/{id}/run` that queued it — and with it, that request's
`api.request_completed`. Then `analysis.step_completed` four times with a
duration each, `image.assessed` twice, and `analysis.job_finished` with the
outcome and the total; or `analysis.dead_lettered` with the reason.

What is never on a line: a storage key, a URI, a coordinate, a grade
distribution, an amount, or a measurement of the photograph (spec §54). A
confidence is one number about the model.
