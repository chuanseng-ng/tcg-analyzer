...
def test_the_only_other_table_is_the_version_record() -> None:
    """The fourth table is provenance, not catalog content — spec §57.

    It shares this `MetaData` because `database/migrations/env.py` compares
    the whole of it against the database, and a table declared anywhere it
    cannot see is a table the next autogenerated revision proposes dropping.
    """
    assert set(metadata.tables) == {*SPEC_TABLES, VERSION_TABLE}
