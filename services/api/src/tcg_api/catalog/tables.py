...
This module is the catalog's source of truth, and the tables attach to the
service-wide `MetaData` in `tcg_api.tables`, which is what
`database/migrations/env.py` compares a database against. The migration that
created them is checked against these definitions by
`services/api/tests/test_catalog_tables.py`. The fourth table is declared here
rather than in a module of its own because it is catalog provenance; the
analysis schema, which is not, lives in `tcg_api.analysis.tables`.
...
`COLLATE "C"` — the shared `PRINTED` type. That is what makes the total order
`CardRepository.search` promises mean the same thing on the local Compose
database and in CI; `tcg_api.tables` gives the full reasoning.
...
#: Every table this module contributes to the shared `MetaData`. Declared so
#: that `test_catalog_tables.py` can still assert nothing sneaked into the
#: schema, now that the `MetaData` is shared with another domain.
TABLES: Final = (sets, cards, card_external_ids, card_database_versions)
