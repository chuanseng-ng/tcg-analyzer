...
| `app/consent/`   | `/consent` — withdraw a photograph kept for training, with the code that was shown once (#148, ADR 0008). No session is read: §54 has deleted the one that produced it                                                                                                                                               |
| `components/`    | `Container` and `Stack` layout primitives, and `ApiStatus`                                                                                                                                                                                                                                                           |
| `lib/`           | `api.ts` — the client for `services/api`; plus `card-*.ts`, `identification.ts`, `upload-*.ts`, `confirm-errors.ts`, `economics-errors.ts`, `results-errors.ts`, `feedback-errors.ts`, `consent-errors.ts`, `results-copy.ts`, `condition-copy.ts`, `analysis-state.ts`, `amount-input.ts` and `analysis-session.ts` |
