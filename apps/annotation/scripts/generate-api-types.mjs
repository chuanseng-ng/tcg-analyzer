 ...
 * this the only sanctioned way for `apps/annotation` to learn an API shape, and it is
...
const outputFile = join(appRoot, "lib", "api-types.ts");
...
if (check) {
  ...
  try {
    ...
    fail("lib/api-types.ts does not exist. Run `pnpm --filter @tcg/annotation gen:api-types`.");
  }
  ...
  if (normalise(existing) !== normalise(generated)) {
    fail(
      "lib/api-types.ts is out of date with the API's OpenAPI schema.\n" +
        "  Run `pnpm --filter @tcg/annotation gen:api-types` and commit the result.",
    );
  }
